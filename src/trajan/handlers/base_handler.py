...
class BASE():
    ...
    def get_box_periods(self):
        if self.__input_file_type not in ["TRAJECTORY"]:
            self.verbose_print(f"ERROR: Input file type \"{self.__input_file_type}\" does not contain periodicity information.")
            sys.exit(1)
